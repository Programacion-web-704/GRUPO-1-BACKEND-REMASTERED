...
export default async function registroAPIFile(req, res) {
  if (req.method !== 'POST') {
    ...
    try {
      ...
      let data = [];
      try {
        const fileContents = await fsPromises.readFile('./json/usuarios.json', 'utf-8'); //ruta al json
      ...
      await fsPromises.writeFile('./json/usuarios.json', JSON.stringify(data, null, 2), 'utf-8');
